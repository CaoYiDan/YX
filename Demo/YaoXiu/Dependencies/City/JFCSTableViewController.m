...
//
    ...
    navView.backgroundColor = ColorBlack;
    ...
    if (indexPath.section < self.dataMutableArray.count) {
        if (indexPath.row < [self.dataMutableArray[indexPath.section] count]) {
            ...
            if (self.headerCitiesMutableArray.count > 0 && [self.sectionFirstLetterMutableArrary[indexPath.section] isEqualToString:kHeaderCitiesSectionIndex]) {//切换城市
                ...
                cell.textLabel.textColor = m_Color_gray(156);
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    header.backgroundColor = ColorBlack;
    
    ls_label( title, self.sectionFirstLetterMutableArrary[section], m_Color_gray(156), m_FontPF_Medium_WithSize(14), ColorBlack, 0, header, {
        make.left.offset(KMargin);
        makeTopOffSet(header, 0);
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.contentInset = UIEdgeInsetsMake(SAFE_NAVBAR_HEIGHT, 0, 0, 0);
...
- (JFCSTableViewHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[JFCSTableViewHeaderView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 40+35)];
        JFWeakSelf(self);
        
        [_headerView headerViewBlock:^(BOOL selected) {
            JFStrongSelf(self);
            [self selectCityCallBack:[_dataOpreation currentCity]];
        }];
    }
    return _headerView;
}
