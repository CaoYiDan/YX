...
//    TCVideoPublishController *vc = [[TCVideoPublishController alloc] initWithPath:result.media.videoPath
//                                                                            videoMsg:[TXVideoInfoReader getVideoInfoWithAsset:result.media.videoAsset]];
//
//       [editViewController.navigationController pushViewController:vc animated:YES];
//    return;
//    
    __weak __typeof(self) wself = self;
...
//        [wself showEditFinishOptionsWithResult:result editController:weakEditController finishBloack:finish];
          __strong __typeof(wself) self = wself; if (!self) return;
         self->_actionAfterSave = TCVideoActionNext;
          finish(YES);
        ...
        if (result.cancelled) {
            ...
            switch(self->_actionAfterSave) {
                ...
                case TCVideoActionPublish:
                    ...
                    break;
                case TCVideoActionNext:
                    [self pushToPublishVC:result nav:weakNavigation];
                    break;
                default:
                    break;
            }
...
-(void)pushToPublishVC:(UGCKitResult *)result nav:(UINavigationController *)nav{
    TCVideoPublishController *vc = [[TCVideoPublishController alloc] initWithPath:result.media.videoPath
    videoMsg:[TXVideoInfoReader getVideoInfoWithAsset:result.media.videoAsset]];
    [nav pushViewController:vc animated:YES];
}
...
-(void)onPublishComplete:(TXPublishResult*)result
{
    ...
    if (result.retCode != 0) {
        ...
        NSLog(@"%@",dictParam);
        [[HttpRequest sharedClient]postWithUrl:YXUserVideo body:dictParam success:^(NSDictionary *response) {
             [_viewController dismissViewControllerAnimated:YES completion:nil];
