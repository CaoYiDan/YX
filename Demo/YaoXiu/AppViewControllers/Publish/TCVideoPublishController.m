...
//    [self.navigationController.navigationBar setTitleTextAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:18],NSForegroundColorAttributeName:[UIColor blackColor]}] ;
    ...
    navView.backgroundColor = m_Color_gray(40);
    [self.view addSubview:navView];
    
    UILabel *lab = [[UILabel alloc]initWithFrame:CGRectMake(0, SafeAreaStatusHeight, ScreenWidth, 44)];
    lab.font = m_FontPF_Regular_WithSize(17);
    lab.text = @"发布";
    ...
    UIButton *backBtn = [[UIButton alloc]initWithFrame:CGRectMake(8, SafeAreaStatusHeight, 44, 44)];
    [backBtn setImage:[UIImage imageNamed:@"backIcon"] forState:0];
    ...
    _labLeftWords.hidden = YES;
    
    _vSharePlatform = [[UIView alloc] init];
    _vSharePlatform.backgroundColor = [UIColor blackColor];
...
//    [_vShare addSubview:share_promise];
    ...
    _labPublishState.textColor = ColorBlack;
    _labPublishState.font = [UIFont systemFontOfSize:16];
    ...
    _labRecordVideo.textColor = ColorBlack;
    ...
    UIButton *publishBtn = [[UIButton alloc]initWithFrame:CGRectMake(50, ScreenHeight-64-SafeAreaBottomHeight, ScreenWidth-100, 40)];
      [publishBtn setTitle:@"发布" forState:0];
    ls_cornerRadius(publishBtn, 6);
    publishBtn.backgroundColor = KMainYellowColor;
      [publishBtn addTarget:self action:@selector(videoPublish) forControlEvents:UIControlEventTouchUpInside];
...
-(void)addLocationView{
   ...
   ls_view(base, ColorClear, self.view, {
       make.left.offset(0);
       makeTopOffSet(self.view, 300);
       make.width.offset(ScreenWidth);
       make.height.offset(60);
   })
    
    ls_imageView(location, @"location", baseView, {
    ...
    ls_label( left, @"", m_Color_gray(231), m_FontPF_Regular_WithSize(15),ColorClear, 0, baseView, {
        make.left.offset(42.5);
        makeTopOffSet(self, 0);
        make.bottom.offset(0);
    })

    ls_imageView(arrow, @"city_arrow", baseView, {
    ...
    WeakSelf;
    ls_addTapGestureRecognizer(baseView, {
        YXLocationListVC *vc = [[YXLocationListVC alloc]init];
        vc.block = ^(ZHPlaceInfoModel *model) {
    ...
    [[SLLocationHelp sharedInstance]getLocationPlacemark:^(CLPlacemark *placemark) {
                  leftLabel.text = placemark.thoroughfare;
...
//    [self.navigationController setNavigationBarHidden:NO];
    [_txtShareWords becomeFirstResponder];
    _playEnable = YES;
    if (_isPublished == NO) {
...
-(void) onPublishComplete:(TXPublishResult*)result
{
   ...
   NSDictionary* dataDic= @{@"user_id" :[TCLoginParam shareInstance].identifier,
   @"video_fileId" : result.videoId,
   @"video_title":title,
   @"video_cover_url":result.coverURL == nil ? @"" : result.coverURL,
   @"video_play_url":result.videoURL};
   
   NSDictionary* dictParam =                                 @{@"action":@"add",
                @"data":dataDic,@"access_token":@"941eedc64f1511a967c38fa728530dae",
                  };
   NSLog(@"%@",dictParam);
   [[HttpRequest sharedClient]postWithUrl:YXUserVideo body:dictParam.mutableCopy success:^(NSDictionary *response) {
        _isPublished = YES;
