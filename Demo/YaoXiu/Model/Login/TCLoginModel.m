...
- (void)getVodSign:(void (^)(int, NSString *, NSDictionary *))completion
{
    ...
    [dic setObject:@"sign" forKey:@"action"];
  
    [[HttpRequest sharedClient]postWithUrl:YXTXYSDKSignature body:dic success:^(NSDictionary *response) {
         completion([response[@"status"] intValue], response[@"msg"], response[@"data"]);
