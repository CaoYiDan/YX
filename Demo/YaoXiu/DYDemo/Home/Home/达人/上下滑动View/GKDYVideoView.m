...
- (instancetype)initFromListPushedWithVC:(UIViewController *)vc{
    ...
    if (self = [super init]) {
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(receiveNotification:) name:VideoHaveLoadMoreNotification object:nil];
        ...
        self.scrollView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
            ...
            //发送加载更多数据的请求
            [[NSNotificationCenter defaultCenter]postNotificationName:VideoNeedLoadMoreNotification object:nil];
...
{
    NSLog(@"object__%@",notification.object);
    
   NSArray *list = notification.object;
