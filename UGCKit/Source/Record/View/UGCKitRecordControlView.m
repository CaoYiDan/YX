...
-(void)setupViews
{
    ...
    UIButton *selectedVideoFormLocationBtn = [[UIButton alloc]initWithFrame:CGRectMake(30, 98, 40, BUTTON_CONTROL_SIZE)];
    [selectedVideoFormLocationBtn setTitle:@"上传" forState:0];
    selectedVideoFormLocationBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
    [selectedVideoFormLocationBtn setTitleColor:[UIColor whiteColor] forState:0];
    [selectedVideoFormLocationBtn addTarget:self action:@selector(selectedVideFromLocationClick) forControlEvents:UIControlEventTouchUpInside];
    [bottomContentView addSubview:selectedVideoFormLocationBtn];
...
-(void)selectedVideFromLocationClick{
    
    [[NSNotificationCenter defaultCenter]postNotificationName:@"SelectedVideFromLocationClicked" object:nil];
